<script setup>
  ...
  const filteredData = computed(() => {
    let fullData = [
      ...sponsorData.value.afdian.map((item) => {
        return {
          ...item,
          platform: "afdian",
        };
      }),
      ...sponsorData.value.patreon.map((item) => {
        return {
          ...item,
          platform: "patreon",
        };
    ...
    return fullData.sort((a, b) => {
      return b.amount * b.unit - a.amount * a.unit;
    });
  });
    ...
    <main class="main">
      <div class="summary">
        ...
        <div class="links-wrapper">
          <div class="links">
            <a
              href="https://www.patreon.com/daidr/membership"
              target="_blank"
              rel="noopener noreferrer"
              >{{ t("page.platform.patreon") }}</a
            >
            |
            <a
              href="https://afdian.net/a/daidr"
              target="_blank"
              rel="noopener noreferrer"
              >{{ t("page.platform.afdian") }}</a
            >
          </div>
        </div>
      </div>
      ...
      <template v-else-if="status === 'success'">
        <div
          class="sponsor-item"
          v-for="sponsor of filteredData"
          :key="sponsor.name + sponsor.amount"
        >
          <div class="left">
            <div
              class="avatar"
          ...
          <div class="middle">
            ...
            <div class="platform">
              {{ t(`page.platform.${sponsor.platform}`) }}
            </div>
          </div>
          <div class="right">
            <div class="amount">
              {{ platformUnit[sponsor.platform] }}{{ sponsor.amount }}
            </div>
          </div>
        </div>
      </template>
